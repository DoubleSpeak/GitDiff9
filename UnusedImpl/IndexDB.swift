...
class IndexDB {
    ...
    func lookup( filePath: String ) -> (Int, Int, Int)? {
        ...
        guard let fileid = filenames[filename], let fileID = filenames[fileName],  let dirID = directories[directory] else {
            print("Could not lookup \(filename) \(String(describing: filenames[filename])) - \(fileName) \(String(describing: filenames[fileName])) - \(directory) \(String(describing: directories[directory]))")
            return nil
        }
    ...
    func usrInFile( filePath: String, line: Int, col: Int ) -> String? {
        ...
        guard select( sql: referenceSQL + " union " + symbolSQL,
            ...
            print( "Refactorator: Found USR #\(usrID) -- \(String(describing: usr))\n\(String(describing: demangle( usr ))) : usr)" )
        } ) else {
